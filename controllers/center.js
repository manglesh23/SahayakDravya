...
const createCenter = async (req, res) => {
  try {
    ...
    if (!centerName || !city || !phone || !pincode) {
    ...
    res
      .status(200)
      .json({ message: "Center Created Successfully", Center: createCenter });
  } catch (e) {
    return {
      error: true,
      details: e,
...
const findCenter = async (req, res) => {
  try {
    let { phone } = req.body;
    let findCenter = await Center.findOne({ phone: "8908908903" }).populate(
      "manager"
    );
